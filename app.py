    ...
    try:
        ...
        with mode_sel:
            manual_max = st.number_input(
                ...
                help="CRITICAL: Used to convert raw marks into percentages. If wrong, the bell curve will be incorrect."
            )
        ...
        # 5. DETAILED TABLES (HTML for easy Copy/Paste)
        st.divider()
        ...
        with st.expander("⚠️ View PA (Pass) Students", expanded=True):
            ...
            if not pa_df.empty:
                ...
                pa_display = pa_df[[s_num_col, 'Student', 'Raw_Original', 'Raw_Adjusted', 'Cat_Adjusted']].copy()
                pa_display.columns = ['S-Number', 'Name', 'Old Mark', 'New Mark', 'Grade']
                st.write("Tip: Drag mouse to select rows and copy.")
                st.table(pa_display)
            else:
        ...
        # Assign to export dataframe safely
